...
| :---: | --- | --- | --- |
| **0** | `0-malloc_checked.c` | Allocates memory using `malloc` and terminates process with status 98 if it fails. | <ul><li>[ ] In Progress</li></ul> |
| **1** | `1-string_nconcat.c` | Concatenates two strings by taking `s1` and the first `n` bytes of `s2`. | <ul><li>[ ] In Progress</li></ul> |
| **2** | `2-calloc.c` | Allocates memory for an array using `malloc` and initializes all bytes to zero. | <ul><li>[ ] In Progress</li></ul> |
| **3** | `3-array_range.c` | Creates an array of integers containing all values from `min` to `max` (inclusive). | <ul><li>[ ] In Progress</li></ul> |
